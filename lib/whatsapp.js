...
export async function sendWithRetry(businessId, jid, text, maxRetries = 5) {
...
export async function initWhatsAppSocket(businessId) {
  ...
  socket.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      const rawId = remoteJid.split("@")[0].split(":")[0];
      const text = msg.message?.conversation || msg.message?.extendedTextMessage?.text || msg.message?.imageMessage?.caption;

      if (!text) continue;
      console.log(`[WA:${businessId}] 📨 Kutoka ${rawId}: "${text.substring(0, 50)}"`);

      handleIncomingMessage({
        businessId,
        remoteJid,
        rawId,
        customerName: msg.pushName || rawId,
        text,
...
async function handleIncomingMessage({ businessId, remoteJid, rawId, customerName, text, platformMessageId }) {
  try {
    ...
    // ✅ FIX: Tumia remoteJid kamili kama customerId ili Dashboard iijue
    const customerId = remoteJid; 
    const customerPhone = rawId; // Namba bila domain
    ...
    const conversation = await prisma.conversation.upsert({
      ...
      create: {
        externalId, businessId, channelType: "WHATSAPP", 
        customerId, 
        customerPhone,
    ...
    if (sent) console.log(`[WA:${businessId}] ✓ AI responded to ${rawId}`);
