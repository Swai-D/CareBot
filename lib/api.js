...
export function getValidToken() {
  ...
  try {
    // Decode bila verify (tu kuangalia payload)
    const payload = JSON.parse(atob(token.split(".")[1]));
    
    // Kama token haina businessId — futa na redirect login
    if (!payload.businessId) {
      console.warn("[Auth] Token ya zamani haina businessId — inafutwa");
      localStorage.removeItem("token");
      window.location.href = "/auth";
      return null;
    }

    // Kama token imeisha muda
    if (payload.exp && payload.exp * 1000 < Date.now()) {
      console.warn("[Auth] Token imeisha muda");
      localStorage.removeItem("token");
      window.location.href = "/auth";
      return null;
    }
...
async function request(path, options = {}) {
  const token = getValidToken(); // Use valid token check
...
export const api = {
  ...
  agent: {
    ...
    test: (body) => request("/agent/test", { method: "POST", body }),
